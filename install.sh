...
#   ./install.sh --no-link      # do not create ~/.local/bin/claw symlink
...
#   CLAW_LINK_BIN=0                    same as --no-link
#   CLAW_BIN_DIR=/custom/bin           symlink destination directory
...
Options:
  ...
  --no-link       Do not create or update ~/.local/bin/claw.
  -h, --help      Show this help text and exit.

Environment overrides:
  CLAW_BUILD_PROFILE   debug | release
  CLAW_SKIP_VERIFY     set to 1 to skip verification
  CLAW_LINK_BIN        set to 0 to skip creating a PATH symlink
  CLAW_BIN_DIR         directory where the claw symlink should be created
EOF
}
...
LINK_BIN="${CLAW_LINK_BIN:-1}"
DEFAULT_BIN_DIR="${HOME:-${PWD}}/.local/bin"
INSTALL_BIN_DIR="${CLAW_BIN_DIR:-${DEFAULT_BIN_DIR}}"
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}6. 'claw' not found after install${COLOR_RESET}
     ...
     The installer also tries to create:
       ${INSTALL_BIN_DIR}/claw
     If that directory is not on PATH, add it in your shell profile.
...
# ---------------------------------------------------------------------------
# Step 5: install a PATH launcher
# ---------------------------------------------------------------------------

step "Installing the claw launcher"

LINK_TARGET="${INSTALL_BIN_DIR}/claw"
LINK_STATUS="not requested"

if [ "${LINK_BIN}" = "1" ]; then
    mkdir -p "${INSTALL_BIN_DIR}"
    if [ -e "${LINK_TARGET}" ] && [ ! -L "${LINK_TARGET}" ]; then
        warn "skipped launcher install because ${LINK_TARGET} already exists and is not a symlink"
        LINK_STATUS="skipped (existing file)"
    else
        ln -sfn "${CLAW_BIN}" "${LINK_TARGET}"
        ok "linked ${LINK_TARGET} -> ${CLAW_BIN}"
        LINK_STATUS="${LINK_TARGET}"
        case ":${PATH}:" in
            *:"${INSTALL_BIN_DIR}":*)
                ok "${INSTALL_BIN_DIR} is already on PATH"
                ;;
            *)
                warn "${INSTALL_BIN_DIR} is not on PATH yet; add it to your shell profile"
                ;;
        esac
    fi
else
    warn "launcher install skipped (--no-link or CLAW_LINK_BIN=0)"
fi
...
${COLOR_GREEN}Claw Code is built and ready.${COLOR_RESET}
  ...
  Launcher: ${LINK_STATUS}

Try it out:

  ${COLOR_DIM}# interactive REPL (from any directory if ${INSTALL_BIN_DIR} is on PATH)${COLOR_RESET}
  claw
