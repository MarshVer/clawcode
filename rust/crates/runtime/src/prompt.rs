...
///
/// Prefers an explicit `CLAW_CURRENT_DATE` override, then the platform `date`
/// command so the value reflects the user's local timezone, and finally falls
/// back to a UTC date derived from `SystemTime` when shelling out is not
/// available.
#[must_use]
pub fn current_local_date() -> String {
    if let Ok(value) = std::env::var("CLAW_CURRENT_DATE") {
    ...
    SystemTime::now()
        ...
        .ok()
        .map(|duration| format_utc_date_from_days((duration.as_secs() / 86_400) as i64))
        .unwrap_or_else(|| "unknown".to_string())
}

fn format_utc_date_from_days(days_since_unix_epoch: i64) -> String {
...
impl SystemPromptBuilder {
    ...
    #[must_use]
    pub fn with_active_model(mut self, active_model: impl Into<String>) -> Self {
        self.active_model = Some(active_model.into());
        self
    }
...
",
    ...
    #[test]
    fn renders_system_prompt_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".claw")).expect("claw dir");
        ...
        let prompt = SystemPromptBuilder::new()
            ...
            .with_active_model("deepseek-ai/DeepSeek-V3.2")
            .with_os("linux", "6.8")
            .with_project_context(project_context)
            .with_runtime_config(config)
            .render();
        ...
        assert!(prompt.contains("Active model: deepseek-ai/DeepSeek-V3.2"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
