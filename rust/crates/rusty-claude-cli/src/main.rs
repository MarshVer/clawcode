...
use runtime::{
    check_base_commit, clear_oauth_credentials, current_local_date, format_stale_base_warning,
...
const BINARY_STARTUP_CONFIG_ENV: &str = "CLAW_STARTUP_CONFIG";
const GENERAL_MODEL_ENV: &str = "CLAW_MODEL";
...
struct LoadedBinaryStartupConfig {
    path: PathBuf,
    config: BinaryStartupConfig,
}
...
fn binary_startup_config_candidates() -> Vec<PathBuf> {
    if let Some(path) = env::var_os(BINARY_STARTUP_CONFIG_ENV).map(PathBuf::from) {
        return vec![path];
    }
...
fn binary_startup_config_candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in exe_dir.ancestors() {
        let candidate = ancestor.join(".claw").join("config.json");
...
) -> Result<Option<LoadedBinaryStartupConfig>, Box<dyn std::error::Error>> {
    for path in binary_startup_config_candidates() {
        let contents = match fs::read_to_string(&path) {
            Ok(contents) => contents,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => continue,
            Err(error) => {
                return Err(format!(
                    "failed to read startup config {}: {error}",
                    path.display()
                )
                .into())
            }
        ...
        let config: BinaryStartupConfig = serde_json::from_str(&contents).map_err(|error| {
            format!(
                "failed to parse startup config {}: {error}",
                path.display()
            )
        })?;
        return Ok(Some(LoadedBinaryStartupConfig { path, config }));
    }
    Ok(None)
}

fn provider_env_names_for_startup_config(
    provider: Option<&str>,
    model: Option<&str>,
) -> Result<Option<(&'static str, &'static str)>, Box<dyn std::error::Error>> {
    if let Some(provider) = provider
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(|value| value.to_ascii_lowercase())
    {
        let env_names = match provider.as_str() {
            "anthropic" => ("ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL"),
            "openai" | "openai-compat" | "openai_compat" | "ollama" | "lm-studio" => {
                ("OPENAI_API_KEY", "OPENAI_BASE_URL")
            }
            "xai" => ("XAI_API_KEY", "XAI_BASE_URL"),
            "dashscope" | "qwen" => ("DASHSCOPE_API_KEY", "DASHSCOPE_BASE_URL"),
            other => {
                return Err(format!(
                    "unsupported startup config provider `{other}`; use anthropic, openai, xai, or dashscope"
                )
                .into())
            }
        };
        return Ok(Some(env_names));
    }

    if let Some(model) = model {
        if let Some(metadata) = metadata_for_model(model) {
            return Ok(Some((metadata.auth_env, metadata.base_url_env)));
        }
        return Ok(Some(("OPENAI_API_KEY", "OPENAI_BASE_URL")));
    }

    Ok(None)
}

fn set_env_if_missing_or_empty(key: &str, value: &str) {
...
fn apply_binary_startup_config() -> Result<(), Box<dyn std::error::Error>> {
    let Some(loaded) = load_binary_startup_config()? else {
        return Ok(());
    };

    let model = normalize_optional_text(loaded.config.model.as_deref())
        .map(|value| api::resolve_model_alias(&value));
    let url = normalize_optional_text(loaded.config.url.as_deref());
    let key = normalize_optional_text(loaded.config.key.as_deref());
    let provider = normalize_optional_text(loaded.config.provider.as_deref());
    let env_names =
        provider_env_names_for_startup_config(provider.as_deref(), model.as_deref()).map_err(
            |error| format!("{}: {error}", loaded.path.display()),
        )?;

    if let Some((auth_env, base_url_env)) = env_names {
        if let Some(key) = key.as_deref() {
            set_env_if_missing_or_empty(auth_env, key);
        }
        if let Some(url) = url.as_deref() {
            set_env_if_missing_or_empty(base_url_env, url);
        }
    }
    if !has_non_empty_env(GENERAL_MODEL_ENV) && !has_non_empty_env("ANTHROPIC_MODEL") {
        if let Some(model) = model.as_deref() {
            set_env_if_missing_or_empty(GENERAL_MODEL_ENV, model);
        }
    }
...
fn resolve_repl_model(cli_model: String) -> String {
    ...
    if let Some(env_model) = env::var(GENERAL_MODEL_ENV)
        .ok()
...
fn parse_system_prompt_args(
    ...
    let mut date = current_local_date();
    let mut index = 0;
...
fn render_doctor_report() -> Result<DoctorReport, Box<dyn std::error::Error>> {
    ...
    let project_context = ProjectContext::discover_with_git(&cwd, current_local_date())?;
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let project_context = ProjectContext::discover(&cwd, current_local_date())?;
    let mut lines = vec![format!(
        "Memory
  Working directory {}
  Instruction files {}",
...
mod tests {
    ...
    #[test]
    fn resolve_repl_model_prefers_general_model_env_when_present() {
        let _guard = env_lock();
        std::env::set_var(GENERAL_MODEL_ENV, "qwen-plus");
        std::env::set_var("ANTHROPIC_MODEL", "sonnet");

        let resolved = resolve_repl_model(DEFAULT_MODEL.to_string());

        assert_eq!(resolved, "qwen-plus");

        std::env::remove_var(GENERAL_MODEL_ENV);
        std::env::remove_var("ANTHROPIC_MODEL");
    }
    ...
    #[test]
    fn binary_startup_config_sets_openai_defaults_for_unknown_model() {
        let _guard = env_lock();
        let root = temp_dir();
        let config_path = root.join(".claw").join("config.json");
        fs::create_dir_all(config_path.parent().expect("parent dir")).expect("config dir");
        fs::write(
            &config_path,
            r#"{
  ...
  "key": "local-test-key"
        ...
        std::env::set_var(BINARY_STARTUP_CONFIG_ENV, &config_path);
        std::env::remove_var(GENERAL_MODEL_ENV);
        std::env::remove_var("OPENAI_BASE_URL");
        std::env::remove_var("OPENAI_API_KEY");

        apply_binary_startup_config().expect("startup config should apply");

        assert_eq!(
            std::env::var(GENERAL_MODEL_ENV).ok().as_deref(),
    ...
    #[test]
    fn binary_startup_config_uses_provider_metadata_when_model_is_known() {
        let _guard = env_lock();
        let root = temp_dir();
        let config_path = root.join(".claw").join("config.json");
        fs::create_dir_all(config_path.parent().expect("parent dir")).expect("config dir");
  ...
  "key": "dashscope-test-key"
        ...
        assert_eq!(
            std::env::var("DASHSCOPE_BASE_URL").ok().as_deref(),
            Some("https://dashscope.example/v1")
        );
        assert_eq!(
            std::env::var("DASHSCOPE_API_KEY").ok().as_deref(),
            Some("dashscope-test-key")
        );
    ...
    #[test]
    fn binary_startup_config_candidates_start_from_executable_directory() {
        let exe = PathBuf::from("/tmp/claw-build/target/debug/claw");

        let candidates = binary_startup_config_candidates_for_exe(&exe);

        assert_eq!(
            candidates.first(),
            Some(&PathBuf::from("/tmp/claw-build/target/debug/.claw/config.json"))
        );
