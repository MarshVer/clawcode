...
use api::{
    dynamic_max_tokens_for_request, resolve_model_alias, ApiError, ContentBlockDelta,
    ...
    ToolDefinition, runtime_messages_to_input_messages,
};
...
use runtime::{
    ...
    glob_search_in_workspace, grep_search_in_workspace, current_local_date, load_system_prompt,
    ...
    LaneEventStatus, LaneFailureClass, McpDegradedReport, PermissionMode,
...
mod tool_budget;
mod tool_specs;
use tool_budget::{budget_edit_file_output, budget_read_file_output, budget_write_file_output};
pub use tool_specs::{deferred_tool_specs, mvp_tool_specs, ToolSpec};
...
fn run_mcp_tool(input: McpToolInput) -> Result<String, String> {
    ...
    match registry.call_tool(&input.server, &input.tool, &args) {
        Ok(result) => to_pretty_json(json!({
            "server": input.server,
            "tool": input.tool,
            "result": budget_mcp_json_result(result),
            "status": "success"
        })),
        Err(e) => to_pretty_json(json!({
            "server": input.server,
...
fn budget_mcp_json_result(result: serde_json::Value) -> serde_json::Value {
    let Ok(rendered) = serde_json::to_string(&result) else {
        return result;
    };
    let budgeted = limit_text_chars(
        &rendered,
        ContextAssemblyBudget::default().mcp_result_chars,
        "MCP tool result",
    );
...
fn run_read_file(input: ReadFileInput) -> Result<String, String> {
    ...
    let output =
        read_file_in_workspace(&input.path, input.offset, input.limit, &cwd).map_err(io_to_string)?;
    to_pretty_json(budget_read_file_output(output))
}

#[allow(clippy::needless_pass_by_value)]
fn run_write_file(input: WriteFileInput) -> Result<String, String> {
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    let output = write_file_in_workspace(&input.path, &input.content, &cwd).map_err(io_to_string)?;
    to_pretty_json(budget_write_file_output(output))
}

#[allow(clippy::needless_pass_by_value)]
fn run_edit_file(input: EditFileInput) -> Result<String, String> {
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    let output = edit_file_in_workspace(
        &input.path,
        &input.old_string,
        &input.new_string,
    ...
    to_pretty_json(budget_edit_file_output(output))
}
...
fn run_glob_search(input: GlobSearchInputValue) -> Result<String, String> {
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    to_pretty_json(
        glob_search_in_workspace(&input.pattern, input.path.as_deref(), &cwd)
            .map_err(io_to_string)?,
    )
}
...
fn run_grep_search(input: GrepSearchInput) -> Result<String, String> {
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    to_pretty_json(grep_search_in_workspace(&input, &cwd).map_err(io_to_string)?)
}
    ...
    let runtime_messages = messages
        ...
        .map(|message| {
            let role = match message.role {
                runtime::MessageRole::System => ApiRuntimeMessageRole::System,
                runtime::MessageRole::User => ApiRuntimeMessageRole::User,
                runtime::MessageRole::Assistant => ApiRuntimeMessageRole::Assistant,
                runtime::MessageRole::Tool => ApiRuntimeMessageRole::Tool,
            };
            let blocks = message
                .blocks
                .iter()
...
mod tests {
  ...
  "nbformat_minor": 5
    ...
    fn file_tools_cover_read_write_and_edit_behaviors() {
        ...
        assert_eq!(write_update_output["originalBytes"], 17);
        ...
        assert_eq!(read_full_output["file"]["contentBytes"], 16);
